...
export default function initUmbraco(args) {
    ...
    const solutionFilePath = join(rootDir, 'kraftvaerk.umbraco.blockfilter.sln');
    ...
    try {
        ...
        execSync(`dotnet add "${join(projectDir, projectName + '.csproj')}" reference "${join(rootDir, 'kraftvaerk.umbraco.blockfilter.Backend', 'kraftvaerk.umbraco.blockfilter.Backend.csproj')}"`, { stdio: 'inherit' });
